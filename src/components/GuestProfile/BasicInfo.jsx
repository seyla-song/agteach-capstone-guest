...
import { useForm } from "react-hook-form";
import { useGetUserInfoQuery, useUpdateInfoMutation } from "../../services/api/userApi";
...
function BasicInfo() {
  ...
  useEffect(() => {
    if (data) {
      ...
      setValue("city", customerData.location_id || "");
      setValue("address", customerData.address || "");
      setSelectedCity(city.find((c) => c.label === customerData.location_id) || null);
    }
  }, [data, setValue]);
  ...
  const onSubmit = async (formData) => {
    ...
    const updatedData = {
      ...
      lastName,
      phone,
      location_id: city, // Assuming location_id is the expected field name in your API
      address,
    };
